...
Loja: escolher personagem base + níveis de arma e escudo (1–5) e alocar atributos

Batalha: turnos alternados com Attack / Defend / Special

Vence quem zerar o HP do oponente
...
Rola ATK efetivo vs DEF efetiva com pequena variação aleatória.

Esquiva → dano 0.

Crítico → aplica multiplicador (ex.: dano *= CRIT×).

Defend no alvo → dano / 2 (no próximo golpe recebido).

Efeitos como sangramento causam dano no início do turno do afetado por N turnos.
...
Opção A — Eclipse (recomendado para você)

Importe o projeto no Eclipse.

Adicione o JavaFX SDK ao Build Path.

Nas VM arguments da run configuration, inclua:

--module-path /caminho/para/javafx/lib --add-modules javafx.controls,javafx.fxml,javafx.graphics


Rode a classe principal (ex.: Game.java ou GameFX.java).
...
Arte: crie spritesheets idle.png, attack1/2/3.png em /assets/characters/<nome>/.

Registro: adicione o personagem na fábrica/lista de personagens jogáveis.

Atributos base: defina ATK/DEF/HP/CRIT/EVA iniciais.

Habilidade especial: implemente um método/efeito (ex.: buff no 1º golpe, counter, bleed aumentado…).

Testes: rode um duelo curto para validar animações e balance.
...
Abra PR com antes/depois (GIF/print) e notas de balanceamento.
